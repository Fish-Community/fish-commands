    ...
    return _b = /** @class */ (function (_super) {
            ...
            FMap.prototype.displayStats = function (f) {
                ...
                var modeSpecificStats = (0, utils_1.match)(config_1.Gamemode.name(), {
                    attack: "Total runs: ".concat(stats.allRunCount, " (").concat(stats.victories, " wins, ").concat(stats.totalLosses, " losses, ").concat(stats.earlyRTVs, " RTVs)\nOutcomes: ").concat(f.percent(stats.winRate, 1), " wins, ").concat(f.percent(stats.lossRate, 1), " losses, ").concat(f.percent(stats.earlyRTVRate, 1), " RTVs\nAverage playtime: ").concat((0, utils_1.formatTime)(stats.averagePlaytime), "\nShortest win time: ").concat((0, utils_1.formatTime)(stats.shortestWinTime), "\nMost recent win: ").concat(stats.mostRecentWin ? (0, utils_1.formatTimestamp)(stats.mostRecentWin) : "[red]none[]"),
                    survival: "Highest wave reached: ".concat(stats.highestWave, "\nAverage wave reached: ").concat(stats.averageWave, "\nTotal runs: ").concat(stats.allRunCount, " (").concat(stats.earlyRTVs, " RTVs)\nRTV rate: ").concat(f.percent(stats.earlyRTVRate, 1), "\nAverage duration: ").concat((0, utils_1.formatTime)(stats.averagePlaytime), "\nLongest duration: ").concat((0, utils_1.formatTime)(stats.longestTime)),
                    pvp: "Total runs: ".concat(stats.allRunCount, " (").concat(stats.earlyRTVs, " RTVs)\nTeam win rates: ").concat(Object.entries(stats.teamWinRate).map(function (_c) {
                        var _d = __read(_c, 2), team = _d[0], rate = _d[1];
                        return "".concat(team, " ").concat(f.percent(rate, 1));
                    }).join(", "), "\nRTV rate: ").concat(f.percent(stats.earlyRTVRate, 1), "\nAverage match duration: ").concat((0, utils_1.formatTime)(stats.averagePlaytime), "\nShortest match duration: ").concat((0, utils_1.formatTime)(stats.shortestWinTime)),
                    hexed: "Total runs: ".concat(stats.allRunCount, " (").concat(stats.earlyRTVs, " RTVs)\nRTV rate: ").concat(f.percent(stats.earlyRTVRate, 1), "\nAverage match duration: ").concat((0, utils_1.formatTime)(stats.averagePlaytime), "\nShortest match duration: ").concat((0, utils_1.formatTime)(stats.shortestWinTime)),
                    sandbox: "Total plays: ".concat(stats.allRunCount, "\nAverage play time: ").concat((0, utils_1.formatTime)(stats.averagePlaytime), "\nShortest play time: ").concat((0, utils_1.formatTime)(stats.shortestTime)),
                }, "");
                var statsText = modeSpecificStats + "\nLongest play time: ".concat((0, utils_1.formatTime)(stats.longestTime), "\nAverage player count: ").concat(f.number(stats.averageHighestPlayerCount, 1));
                return ("[coral]".concat(map.name(), "\n[gray](").concat(map.file.name(), ")\n\n[accent]Map by: [white]").concat(map.author(), "\n[accent]Description: [white]").concat(map.description(), "\n[accent]Size: [white]").concat(map.width, "x").concat(map.height, "\n[accent]Last updated: [white]").concat(new Date(map.file.lastModified()).toLocaleDateString(), "\n[accent]BvB allowed: ").concat(f.boolGood(rules.placeRangeCheck), ", unit item transfer allowed: ").concat(f.boolGood(rules.onlyDepositCore), "\n\n").concat(statsText.split('\n').map(function (r) { return config_1.FColor.mapStats() + r; }).join('\n')));
            };
            return FMap;
        }(_classSuper)),
        (function () {
