...
exports.manualAfk = exports.sender = exports.afk = exports.commands = void 0;
var api = require("/api");
...
exports.commands = (0, commands_1.commandList)({
    ...
    aoelog: (0, commands_1.command)(function () {
        ...
        return {
            ...
            tapped({ x: x, y: y, output: output, sender: exports.sender, admins: admins, handleTaps: handleTaps, args: args, currentTapMode: currentTapMode });
        {
            function handleArea(p1, p2) {
                ...
                var limitTiles = 0;
                var amount = Math.floor(Math.abs(Number(args.amount))) || 10;
                    ...
                    for (var j = minY; j <= maxY; j++) {
                        ...
                        output("[yellow]Tile history for tile (".concat(i, ", ").concat(j, "):\n") + history.map(function (e) {
                            ...
                            if (globals_1.uuidPattern.test(e.uuid)) {
                                if (exports.sender.hasPerm("viewUUIDs"))
...
}), exports.sender = (_a = void 0, _a.sender), exports.manualAfk = (_b = _a.manualAfk, _b === void 0 ? !exports.sender.manualAfk : _b);
exports.sender.updateName();
if (exports.sender.manualAfk)
...
vanish: {
    args: ['target:player?'],
        description;
    "Toggles visibility of your rank and flags.",
        perm;
    commands_1.Perm.vanish,
        handler({ sender: exports.sender, args: { target: target }, outputSuccess: outputSuccess });
    {
        if (exports.sender.stelled())
            (0, commands_1.fail)("Marked players may not hide flags.");
        if (exports.sender.muted)
            (0, commands_1.fail)("Muted players may not hide flags.");
        if (exports.sender != target && target.hasPerm("blockTrolling"))
            (0, commands_1.fail)("Target is insufficentlly trollable.");
        if (exports.sender != target && !exports.sender.ranksAtLeast("mod"))
            (0, commands_1.fail)("You do not have permission to vanish other players.");
        target.showRankPrefix = !target.showRankPrefix;
        outputSuccess("".concat(target == exports.sender ? "Your" : "".concat(target.name, "'s"), " rank prefix is now ").concat(target.showRankPrefix ? "visible" : "hidden", "."));
    }
}
tileid: {
    args: [],
    ...
    'Checks id of a tile.',
        perm;
    commands_1.Perm.none,
        handler({ output: output, handleTaps: handleTaps });
    {
        handleTaps("once");
        output("Click a tile to see its id...");
    }
    tapped({ output: output, f: f, tile: tile });
    {
...
}
{
    ...
    "Switches to another server.",
        perm;
    commands_1.Perm.play,
        handler({ args: args, sender: exports.sender, f: f });
    {
        if (args.target != null && args.target != exports.sender && !exports.sender.canModerate(args.target, true, "admin", true))
        ...
        var target = (_d = args.target) !== null && _d !== void 0 ? _d : exports.sender;
        if (globals_1.ipPortPattern.test(args.server) && exports.sender.hasPerm("admin")) {
            //direct connect
            Call.connect.apply(Call, __spreadArray([target.con], __read(args.server.split(":")), false));
        }
        else {
            var unknownServerMessage = "Unknown server ".concat(args.server, ". Valid options: ").concat(config_1.FishServer.all.filter(function (s) { return !s.requiredPerm || exports.sender.hasPerm(s.requiredPerm); }).map(function (s) { return s.name; }).join(", "));
            var server = (_e = config_1.FishServer.byName(args.server)) !== null && _e !== void 0 ? _e : (0, commands_1.fail)(unknownServerMessage);
            //Pretend the server doesn't exist
            if (server.requiredPerm && !exports.sender.hasPerm(server.requiredPerm))
                (0, commands_1.fail)(unknownServerMessage);
            if (target == exports.sender)
                players_1.FishPlayer.messageAllWithPerm(server.requiredPerm, "".concat(exports.sender.name, "[magenta] has gone to the ").concat(server.name, " server. Use [cyan]/").concat(server.name, " [magenta]to join them!"));
...
s: {
    ...
    "Sends a message to staff only.",
        perm;
    commands_1.Perm.chat,
        handler({ sender: exports.sender, args: args, outputSuccess: outputSuccess, outputFail: outputFail, lastUsedSender: lastUsedSender });
    {
        if (!exports.sender.hasPerm("mod")) {
        ...
        api.sendStaffMessage(args.message, exports.sender.name, function (sent) {
            if (!exports.sender.hasPerm("mod")) {
                if (sent) {
                    outputSuccess("Message sent to [orange]all online staff.");
                }
                else {
                    var wasReceived = players_1.FishPlayer.messageStaff(exports.sender.prefixedName, args.message);
                    if (wasReceived)
...
watch: {
    ...
    "Watch/unwatch a player.",
        perm;
    commands_1.Perm.none,
        handler({ args: args, sender: exports.sender, outputSuccess: outputSuccess, outputFail: outputFail });
    {
        if (exports.sender.watch) {
            outputSuccess("No longer watching a player.");
            exports.sender.watch = false;
        }
        else if (args.player) {
            exports.sender.watch = true;
            var stayX_1 = (_f = exports.sender.unit()) === null || _f === void 0 ? void 0 : _f.x;
            var stayY_1 = (_g = exports.sender.unit()) === null || _g === void 0 ? void 0 : _g.y;
            var target_1 = args.player.player;
            var watch_1 = function () {
                var _a, _b;
                if (exports.sender.watch && target_1.unit()) {
                    // Self.X+(172.5-Self.X)/10
                    Call.setCameraPosition(exports.sender.con, target_1.unit().x, target_1.unit().y);
                    (_b = (_a = exports.sender.unit()) === null || _a === void 0 ? void 0 : _a.set) === null || _b === void 0 ? void 0 : _b.call(_a, stayX_1, stayY_1);
                    Timer.schedule(function () { return watch_1(); }, 0.1, 0.1, 0);
                }
                else {
                    Call.setCameraPosition(exports.sender.con, stayX_1, stayY_1);
                }
            };
            watch_1();
        }
    ...
    help;
{
    ...
    'Displays a list of all commands.',
        perm;
    commands_1.Perm.none,
        handler({ args: args, output: output, sender: exports.sender, allCommands: allCommands });
    {
        var formatCommand_1 = function (name, color) {
        ...
        if (args.name && isNaN(parseInt(args.name)) && !['mod', 'admin', 'member'].includes(args.name)) {
            ...
            if (args.name in allCommands && (!allCommands[args.name].isHidden || allCommands[args.name].perm.check(exports.sender))) {
            ...
            switch (args.name) {
                ...
                default:
                    ...
                    var page = (_h = chunkedPlayerCommands[pageNumber - 1]) !== null && _h !== void 0 ? _h : (0, commands_1.fail)("\"".concat(args.name, "\" is an invalid page number."));
...
msg: {
    ...
    'Send a message to only one player.',
        perm;
    commands_1.Perm.chat,
        handler({ args: args, sender: exports.sender, output: output, f: f });
    {
        globals_1.recentWhispers[args.player.uuid] = exports.sender.uuid;
        args.player.sendMessage("".concat(exports.sender.prefixedName, "[lightgray] whispered:[#BBBBBB] ").concat(args.message));
...
r: {
    ...
    commands_1.Perm.chat,
        handler({ args: args, sender: exports.sender, output: output, f: f });
    {
        var recipient = players_1.FishPlayer.getById((_j = globals_1.recentWhispers[exports.sender.uuid]) !== null && _j !== void 0 ? _j : (0, commands_1.fail)("It doesn't look like someone has messaged you recently. Try whispering to them with [white]\"/msg <player> <message>\""));
        ...
        globals_1.recentWhispers[globals_1.recentWhispers[exports.sender.uuid]] = exports.sender.uuid;
        recipient.sendMessage("".concat(exports.sender.name, "[lightgray] whispered:[#BBBBBB] ").concat(args.message));
...
trail: {
    ...
    'Use command to see options and toggle trail on/off.',
        perm;
    commands_1.Perm.none,
        handler({ args: args, sender: exports.sender, output: output, outputFail: outputFail, outputSuccess: outputSuccess });
    {
        //overload 1: type not specified
        if (!args.type) {
            if (exports.sender.trail != null) {
                exports.sender.trail = null;
    ...
    ranks;
{
    args: [],
        description;
    'Displays information about all ranks.',
        perm;
    commands_1.Perm.none,
...
rules: {
    ...
    'Displays the server rules.',
        perm;
    commands_1.Perm.none,
        handler({ args: args, sender: exports.sender, outputSuccess: outputSuccess, f: f });
    {
        var target_2 = (_k = args.player) !== null && _k !== void 0 ? _k : exports.sender;
        if (target_2 !== exports.sender) {
            if (!exports.sender.hasPerm("warn"))
                (0, commands_1.fail)("You do not have permission to show rules to other players.");
            if (target_2.hasPerm("blockTrolling"))
        ...
        menus_1.Menu.menu("Rules for [#0000ff]>|||> FISH [white]servers", config_1.rules.join("\n\n"), ["[green]I agree to abide by these rules[]", "No"], target_2).then(function (option) {
            if (option == "No") {
                target_2.kick("You must agree to the rules to play on this server. Rejoin to agree to the rules.", 1);
        ...
        if (target_2 !== exports.sender)
            outputSuccess(f(templateObject_10 || (templateObject_10 = __makeTemplateObject(["Reminded ", " of the rules."], ["Reminded ", " of the rules."])), target_2));
    }
}
void ;
{
    args: ["player:player?"],
        description;
    'Warns other players about power voids.',
        perm;
    commands_1.Perm.play,
        requirements;
    [commands_1.Req.mode("attack")],
        handler({ args: args, sender: exports.sender, lastUsedSuccessfullySender: lastUsedSuccessfullySender, lastUsedSuccessfully: lastUsedSuccessfully, outputSuccess: outputSuccess, f: f });
    {
        if (args.player) {
            if (Date.now() - lastUsedSuccessfullySender < 20000)
                (0, commands_1.fail)("This command was used recently and is on cooldown.");
            if (!exports.sender.hasPerm("trusted"))
            ...
            if (args.player !== exports.sender && args.player.hasPerm("blockTrolling"))
                (0, commands_1.fail)("Target player is insufficiently trollable.");
            ...
            (0, utils_1.logAction)("showed void warning", exports.sender, args.player);
...
team: {
    ...
    'Changes your team.',
        perm;
    commands_1.Perm.changeTeam,
        handler({ sender: exports.sender, args: { team: team, reason: reason }, outputSuccess: outputSuccess, f: f });
    {
        if (config_1.Gamemode.sandbox() && globals_1.fishState.peacefulMode && !exports.sender.hasPerm("admin"))
            (0, commands_1.fail)("You do not have permission to change teams because peaceful mode is on.");
        if (config_1.Gamemode.sandbox() && team === Vars.state.rules.waveTeam && !exports.sender.hasPerm("admin"))
            (0, commands_1.fail)("You do not have permission to change to the wave team on sandbox.");
        if (!config_1.Gamemode.sandbox() && !exports.sender.hasPerm("mod") && !reason)
            (0, commands_1.fail)("Please specify a reason for changing teams.");
        if (!exports.sender.hasPerm("changeTeamExternal")) {
            if (team.data().cores.size <= 0)
                (0, commands_1.fail)("You do not have permission to change to a team with no cores.");
            if (!exports.sender.player.dead() && !((_l = exports.sender.unit()) === null || _l === void 0 ? void 0 : _l.spawnedByCore))
                exports.sender.forceRespawn();
        }
        if (!exports.sender.hasPerm("mod"))
            exports.sender.changedTeam = true;
        exports.sender.setTeam(team);
        outputSuccess(f(templateObject_13 || (templateObject_13 = __makeTemplateObject(["Changed your team to ", "."], ["Changed your team to ", "."])), team));
        if (reason)
            (0, utils_1.logAction)("changed team to ".concat(team.name, " on ").concat((0, funcs_1.escapeTextDiscord)(Vars.state.map.plainName()), " with reason ").concat((0, funcs_1.escapeTextDiscord)(reason)), exports.sender);
    }
}
teamp: {
    ...
    'Changes the team of a player.',
        perm;
    commands_1.Perm.changeTeam,
        handler({ sender: exports.sender, args: { team: team, target: target }, outputSuccess: outputSuccess, f: f });
    {
        if (!exports.sender.canModerate(target, true, "mod", true))
        ...
        if (!exports.sender.hasPerm("changeTeamExternal")) {
            if (team.data().cores.size <= 0)
                (0, commands_1.fail)("You do not have permission to change to a team with no cores.");
            if (!target.player.dead() && !((_m = target.unit()) === null || _m === void 0 ? void 0 : _m.spawnedByCore))
                target.forceRespawn();
        }
        target.setTeam(team);
...
rank: {
    args: ['player:player'],
        description;
    'Displays the rank of a player.',
        perm;
    commands_1.Perm.none,
        handler({ args: args, output: output, f: f });
    {
...
forcevnw: {
    args: ["force:boolean?"],
        description;
    'Force skip to the next wave.',
        perm;
    commands_1.Perm.admin,
        handler({ allCommands: allCommands, sender: exports.sender, args: { force: force } });
    {
        if (allCommands.vnw.data.manager.session == null) {
            ...
            if (force)
                Call.sendMessage("VNW: [green]Vote was forced by admin [yellow]".concat(exports.sender.name, "[green], skipping wave."));
            else
                Call.sendMessage("VNW: [red]Votes cleared by admin [yellow]".concat(exports.sender.name, "[red]."));
    ...
    forcertv;
{
    args: ["force:boolean?"],
        description;
    'Force skip to the next map.',
        perm;
    commands_1.Perm.admin,
        handler({ args: { force: force }, sender: exports.sender, allCommands: allCommands });
    {
        if (allCommands.rtv.data.manager.session == null) {
            ...
            if (force)
                Call.sendMessage("RTV: [green]Vote was forced by admin [yellow]".concat(exports.sender.name, "[green]."));
            else
                Call.sendMessage("RTV: [red]Votes cleared by admin [yellow]".concat(exports.sender.name, "[red]."));
    ...
    forcenextmap;
{
    args: ["map:map"],
        description;
    'Override the next map in queue.',
        perm;
    commands_1.Perm.admin.exceptModes({
        testsrv: commands_1.Perm.play
    }),
        handler({ allCommands: allCommands, args: args, sender: exports.sender, outputSuccess: outputSuccess, f: f });
    {
        ...
        if (allCommands.nextmap.data.voteEndTime() > -1) {
            ...
            Call.sendMessage("[red]Admin ".concat(exports.sender.name, "[red] has cancelled the vote. The next map will be [yellow]").concat(args.map.name(), "."));
        }
        else {
    ...
    stats;
{
    ...
    commands_1.Perm.none,
        description;
    "Views a player's stats.",
        handler({ args: { target: target }, output: output, f: f });
    {
...
showworld: {
    ...
    commands_1.Perm.none,
        description;
    "Views the world as a 2D scrollable menu.",
        requirements;
    [commands_1.Req.cooldown(4000)],
        handler({ sender: exports.sender, args: { size: size, x: x, y: y } });
    {
        if (size > 20)
        ...
        var height_1 = Vars.world.height();
        menus_1.Menu.scroll(exports.sender, "The World", "Use the arrow keys to navigate around the world. Click a blank square to exit.", options, {
            columns: size,
            rows: size,
            x: x ? x - Math.trunc(size / 2) : 0,
            y: height_1 - (y ? y + 1 + Math.trunc(size / 2) : size),
            getCenterText: function (x, y) { return "".concat(x, ",").concat(height_1 - y - size); }
        });
    }
}
mapinfo: {
    ...
    commands_1.Perm.none,
        description;
    "Displays information about a map.",
        handler({ output: output, args: { map: map }, f: f, sender: exports.sender });
    {
        if (map) {
            output(maps_1.FMap.getCreate(map).displayStats(f));
        }
        else {
            menus_1.Menu.textPages(exports.sender, Vars.maps.customMaps().map(function (m) {
...
gamemode: {
    args: ["mode:string"],
        perm;
    new commands_1.Perm("changeGamemode", "manager").exceptModes({
        testsrv: commands_1.Perm.play,
    }),
        description;
    "Sets the gamemode.",
        requirements;
    [commands_1.Req.cooldownGlobal(10000)],
        handler({ args: args, sender: exports.sender, outputSuccess: outputSuccess });
    {
        if (!exports.sender.hasPerm('trusted'))
...
v8upgrade: {
    ...
    commands_1.Perm.none,
        description;
    "Provides instructions to update to v8.",
        handler({ sender: exports.sender });
    {
        menus_1.Menu.menu("V8 Migration Information", "Where did you download Mindustry?", exports.sender.con.mobile ? [
            "Google Play Store",
            "Apple App Store",
            "itch.io",
            "F-Droid (APK)",
            ...
            "Foo's Client",
            "MindustryLauncher",
        ], exports.sender, { onCancel: 'reject', includeCancel: true }).then(function (response) {
            ...
            exports.sender.sendMessage("[coral]V8 Migration[] for [accent]".concat(response, "[]: ").concat(message));
        });
    }
}
v8pollresults: {
        ...
        handler({ output: output });
    {
        ...
        try {
            for (var _o = __values(Object.values(players_1.FishPlayer.cachedPlayers)), _p = _o.next(); !_p.done; _p = _o.next()) {
                var player = _p.value;
...
highscore: {
    ...
    commands_1.Perm.none,
        description;
    'This command was moved to /mapinfo.',
        handler();
    {
