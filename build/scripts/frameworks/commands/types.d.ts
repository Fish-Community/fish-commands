...
export declare const commandArgTypes: readonly ["string", "number", "boolean", "player", "team", "time", "unittype", "block", "uuid", "offlinePlayer", "map", "rank", "roleflag", "item"];
export declare const commandArgNames: {
    string: string;
    number: string;
    boolean: string;
    player: string;
...
export type ValueFor<ArgString> = ArgString extends `${string}:${infer V}?` ? TypeOfArgType<V> | undefined : ArgString extends `${string}:${infer V}` ? TypeOfArgType<V> : never;
 ...
 */
export type PartialFormatString<TData = string | null> = ((data: TData) => string) & {
...
export type FishCommandData<ArgType extends string, StoredData> = {
    ...
    init?: () => StoredData;
    data?: StoredData;
    requirements?: Array<NoInfer<FishCommandRequirement<ArgType, StoredData>>> | ((_: FishCommandHandlerData<ArgType, StoredData> & FishCommandHandlerUtils) => Array<NoInfer<FishCommandRequirement<ArgType, StoredData>>>);
